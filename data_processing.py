...
def get_geolocator():
    ...
    return Nominatim(user_agent="cdmx-crime-map/1.0")
...
def geocode_address(address):
  try:
    ...
    # Respect rate limit
    time.sleep(1) 
    location = geolocator.geocode(address, country_codes="MX")
    if location:
...
def load_all_years():
  ...
  if df_raw is None:
    print("Failed to load raw data. Aborting.")
    return {}
...
def create_map(year):
  ...
  if not boroughs_coordinates or not metro_coordinates:
      print("Missing geojson coordinate files. Map cannot be fully rendered.")
      # We can still proceed without them, but layers will be missing.
  ...
  try:
    ...
    print(f"Could not load metro stations file: {e}")
    metro_stations_gdf = gpd.GeoDataFrame(columns=['nombre', 'geometry'], crs="EPSG:4326") # Create empty gdf
  ...
  if not metro_proj.empty:
    ...
    # If no metro stations loaded, create an empty frame to avoid errors
    station_df = pd.DataFrame(columns=["nombre", "lat", "lon", "robos"])
  ...
  # Compute robberies near each metro station (if stations were loaded)
  if not metro_stations_gdf.empty:
    station_counts = []
    for idx, station in metro_stations_gdf.iterrows():
      # Use a small buffer for nearby calculation (approx 50m)
      # 0.0005 degrees is roughly 55m
      nearby = crimes_gdf[
        crimes_gdf.geometry.within(station.geometry.buffer(0.0005)) 
      ]  
    ...
    for _, row in metro_stations_gdf.iterrows():
      popup_text = f"<b>{row['nombre']}</b>"
      if 'ubicacion' in row:
          popup_text += f"<br>{row['ubicacion']}"
  ...
  # Add Boroughs layer if loaded
  if boroughs_coordinates:
    try:
      folium.GeoJson(
      ...
      print(f"Could not load boroughs geojson: {e}")
